    ...
    try {
      ...
      if (shouldProxyPrivateDownload && latestJson.platforms) {
        for (const [platform, data] of Object.entries(latestJson.platforms)) {
          // Map Tauri platform names to Hazel platform names
          let hazelPlatform
          if (platform === 'darwin-x86_64') {
            hazelPlatform = 'dmg'
          } else if (platform === 'darwin-aarch64') {
            hazelPlatform = 'dmg_arm64'
          } else if (platform === 'windows-x86_64') {
            hazelPlatform = 'exe'
          } else if (platform === 'linux-x86_64') {
            hazelPlatform = 'AppImage'
          }

          if (hazelPlatform) {
            latestJson.platforms[platform].url = `https://${url}/download/${hazelPlatform}`
          }
        }
      }
