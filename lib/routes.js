...
module.exports = ({ cache, config }) => {
  ...
  exports.latestJson = async (req, res) => {
    try {
      ...
      const latestJsonUrl = `https://github.com/${config.account}/${config.repository}/releases/latest/download/latest.json`
      ...
      const response = await fetch(latestJsonUrl, { headers })
